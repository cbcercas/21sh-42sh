...
void	sh_help_pwd(void)
{
	...
	ft_printf("%s-P%s\tThe pathname written to standard output shall not contain"
					" any components that refer to files of type symbolic"
					" link. \n", C_CYAN, C_NONE);
	ft_printf("If both %s-L%s and %s-P%s are specified, the last one shall apply."
					" If neither %s-L%s nor %s-P%s is specified, the pwd utility"
					" shall behave as if %s-L%s had been specified.\n",
			C_CYAN, C_NONE, C_CYAN, C_NONE, C_CYAN, C_NONE, C_CYAN, C_NONE, C_CYAN, C_NONE);
}
